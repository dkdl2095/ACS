...
<body>
	...
	<table border="1">
		...
		<c:forEach items="${TENANTCOMPLET}" var="s">
			<a href="news.nhn?action=getNews&aid=${s.aid}"
					class="text-decoration-none"> ${s.title},
						${news.date}</a> <a href="news.nhn?action=deleteNews&aid=${s.aid}"><span
						class="badgebg-secoundary"> &times;</span></a>
		</c:forEach>
	</table>
	<h2>회원추가</h2>
	<hr>
	<form method="post" action="/Project/TENANTCOMPLET?action=insert">
